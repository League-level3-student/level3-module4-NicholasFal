...
public class _01_IntroToQueue {
    public static void main(String[] args) {
        ...
        //    Note: you have to use the capitalized Double and not double
    	ArrayDeque<Double> moreDoubles = new ArrayDeque<Double>(); 
        // 4. Pop off 5 elements from the Stack and add them to the Queue 
    	for(int i = 0; i < 5; i++) {
    	moreDoubles.add(doubles.pop());
    	}
    	...
    	while(doubles.size() != 0 && doubles.size() != 0) {
    	int removing = ran.nextInt(5) + 1;
    	for(int i = 0; i < removing; i++) {
    		System.out.println("removing " + removing + " elements from Queue: " + moreDoubles.remove());
    	}
